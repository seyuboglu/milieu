...
class BuildNetworkMatrix(Process):
    ...
    def save(self):
        ...
        np.save(
            os.path.join(self.dir, "protein_to_node.npy"), self.network.name_to_node
        )
    ...
    def build_matrix(self):
        ...
        ci_matrix = (
            csr_matrix((inv_deg_vector * self.network.adj_matrix).T)
            * csr_matrix(self.network.adj_matrix)
        ).toarray()
...
def load_network_matrices(name_to_matrix, network=None):
    ...
    for name, matrix_dir in name_to_matrix.items():
        ...
        protein_to_node = np.load(
            os.path.join(matrix_dir, "protein_to_node.npy"), allow_pickle=True
        )
        assert network is None or network.name_to_node == protein_to_node
